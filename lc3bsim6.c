...
void MEM_stage() {
  ...
  if(Get_DCACHE_RW(PS.MEM_CS)){//if write
    if(Get_DATA_SIZE(PS.MEM_CS)){//word
      we_0=1;
      we_1=1;
    }
    else{//byte
      if((PS.MEM_ADDRESS)%2){//odd
        we_1=1;
      }
      else{
        we_0=1;
  ...
  if(Get_DATA_SIZE(PS.MEM_CS)){
    data_to_write=Low16bits(PS.MEM_ALU_RESULT);
  }
  else{//byte
    if((PS.MEM_ADDRESS)%2){//odd
      data_to_write=Low16bits((PS.MEM_ALU_RESULT) << 8); //align the lower byte to the higher byte in memory
    }
    else{
      data_to_write=Low16bits((PS.MEM_ALU_RESULT) & 0x00FF); //lower byte
    }

  }

  if(v_dcache_en){//dcache access
   dcache_access(PS.MEM_ADDRESS, &data_to_read, data_to_write, &dcache_r, we_0, we_1);
  }
  else{
    data_to_read = 0x0000;
  }
  ...
  if(Get_DATA_SIZE(PS.MEM_CS) == 0){ //byte
    if((PS.MEM_ADDRESS)%2){//odd
      data_to_read=Low16bits(sext8(data_to_read>>8)); //align the lower byte to the higher byte in memory
    }
    else{
      data_to_read=Low16bits(sext8(data_to_read&0x00FF)); //lower byte
    }
  }

  trap_pc=shift_sext_output;

  mem_pcmux=0; //default;

  if(PS.MEM_V){
    if(Get_TRAP_OP(PS.MEM_CS)){ //trap
      mem_pcmux=2;
    }
    else if (Get_UNCOND_OP(PS.MEM_CS)){//jsr/rr
      mem_pcmux=1;
    }
    else if (Get_BR_OP(PS.MEM_CS)){//branch
      if(((PS.MEM_IR) & 0x0200) && ((PS.MEM_CC)&0x01)){//p
        mem_pcmux=1;
      }
      if(((PS.MEM_IR) & 0x0400) && ((PS.MEM_CC)&0x02)){//z
        mem_pcmux=1;
      }
      if(((PS.MEM_IR) & 0x0800) && ((PS.MEM_CC)&0x04)){//n
        mem_pcmux=1;
      }
    }
  }
...
void AGEX_stage() {
  ...
  if(Get_ADDR2MUX(PS.AGEX_CS)==0){
    ...
    adder2=Low16bits(sext6((PS.AGEX_IR & 0x003F)));
  }
  else if(Get_ADDR2MUX(PS.AGEX_CS)==2){
    adder2=Low16bits(sext9((PS.AGEX_IR & 0x01FF)));
  }
  else if(Get_ADDR2MUX(PS.AGEX_CS)==3){
    adder2=Low16bits(sext11((PS.AGEX_IR & 0x07FF)));
  }
  ...
  big_adder=adder1+shifter;

  if(Get_ADDRESSMUX(PS.AGEX_CS)){
    address_m=Low16bits(big_adder);
  }
  else{
    address_m=Low16bits(((PS.AGEX_IR)&0x00FF)<<1);
  }

  if(Get_SR2MUX(PS.AGEX_CS)){
    sr2_m=Low16bits(sext5((PS.AGEX_IR)&0x001F));
  }
  else{
    sr2_m=PS.AGEX_SR2;
  }

  srcA=Low16bits(PS.AGEX_SR1);
  srcB=Low16bits(sr2_m);

  if(Get_ALUK(PS.AGEX_CS) == 0){ //add
    alu_reg=srcA+srcB;
  }
  else if(Get_ALUK(PS.AGEX_CS) == 1){//and
    alu_reg=srcA&srcB;
  }
  else if(Get_ALUK(PS.AGEX_CS) == 2){//xor
    alu_reg=srcA^srcB;
  }
  else if(Get_ALUK(PS.AGEX_CS) == 3){//passb
    alu_reg=srcB;
  }

  int shf_amt = ((PS.AGEX_IR)&0x000F);
  int shf_type = ((PS.AGEX_IR)&0x0030)>>4;

  if(shf_type==0||shf_type==2){//lshf
    shf_reg=Low16bits((PS.AGEX_SR1)<<(shf_amt));
  }
  else if(shf_type==1){//rshfl
    shf_reg=(PS.AGEX_SR1)>>(shf_amt);
  }
  else if(shf_type==3){//rshfa
    if((PS.AGEX_SR1)&0x8000){
      shf_reg = ((PS.AGEX_SR1)>>shf_amt)|(0xFFFF<<(16-shf_amt));
    }
    else{
      shf_reg=(PS.AGEX_SR1)>>shf_amt;
    }
  ...
  if(Get_ALU_RESULTMUX(PS.AGEX_CS)){
    alu_result=Low16bits(alu_reg);
  }
  else{
    alu_result=Low16bits(shf_reg);
  }
...
int MICROINSTRUCTION[NUM_CONTROL_STORE_BITS]; //microcode for given instruction
int sr1;
int sr1_contents;
int sr1_needed;
int sr2;
...
void DE_stage() {
  ...
  for (int i=0; i<NUM_CONTROL_STORE_BITS;i++){
    MICROINSTRUCTION[i]=CONTROL_STORE[CONTROL_STORE_ADDRESS][i];
  }

  v_de_br_stall = PS.DE_V & MICROINSTRUCTION[BR_STALL];
  dep_stall=0;
  LD_AGEX=!mem_stall;

  sr1=(((PS.DE_IR)&0x01C0)>>6);
  if((PS.DE_IR)&0x2000){ //stb or stw
  ...
  cc = (N<<2)|(Z<<1)|P;
  sr1_contents = REGS[sr1];
  sr2_contents = REGS[sr2];

  if(MICROINSTRUCTION[DRMUX]==0){ //sr2mux signal
    dr=(((PS.DE_IR)&0x0E00)>>9);
  }
  else{
    dr=7;
  }

  /*load registers and cc's*/
  if(v_sr_ld_cc){
    N=sr_n;
    Z=sr_z;
    P=sr_p;
  ...
  sr1_needed=MICROINSTRUCTION[SR1_NEEDED];
  sr2_needed=MICROINSTRUCTION[SR2_NEEDED];
  dest_agex=PS.AGEX_DRID;
  dest_mem=PS.MEM_DRID;
  dest_sr=PS.SR_DRID;
  br_op=MICROINSTRUCTION[BR_OP];

 if(PS.DE_V){ //only if instruction is valid does it need to stall pipeline
    if(sr1_needed){
      if(((sr1==dest_agex) && v_agex_ld_reg)||
      ((sr1==dest_mem) && v_mem_ld_reg) ||
      ((sr1==dest_sr) && v_sr_ld_reg)){
        dep_stall=1;
      }
    ...
    if(sr2_needed){
      if(((sr2==dest_agex) && v_agex_ld_reg)||
      ((sr2==dest_mem) && v_mem_ld_reg) ||
      ((sr2==dest_sr) && v_sr_ld_reg)){
        dep_stall=1;
      }
  ...
  if (LD_AGEX) {
    ...
    NEW_PS.AGEX_SR1=sr1_contents;
    NEW_PS.AGEX_SR2=sr2_contents;
    NEW_PS.AGEX_CC=cc;
    NEW_PS.AGEX_DRID=dr;
...
de_npc_temp=PC+2;
  ...
  icache_access(PC, &next_word_memory, &icache_r);

  ld_pc=(icache_r& !dep_stall & !v_de_br_stall & !v_agex_br_stall & !v_mem_br_stall & !mem_stall) || ((mem_pcmux!=0) && v_mem_br_stall); //valid control instruction
  ...
  if(ld_de){
    NEW_PS.DE_NPC=de_npc_temp;
    NEW_PS.DE_IR=next_word_memory;
    NEW_PS.DE_V=icache_r & !v_de_br_stall & !v_agex_br_stall & !v_mem_br_stall; //icache ready and nothing needs to be thrown away
  }
